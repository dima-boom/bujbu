import telebot, vk_api, time, threading, requests, os, vk_captchasolver as vc, psycopg2, random
...
def rass(user_id, group_col):
    ...
    for i in range(int(group_col)//500):
        ...
        for k in range(500):
            ...
            grp+=1
        new_sp = vk.groups.getById(group_ids=int(sp_group), fields="can_message")
        for j in new_sp:
            if j["can_message"] == 1:
